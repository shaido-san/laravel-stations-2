...
<body>
    ...
    <div class="movies-list">
        @foreach ($movies as $movie)
            ...
            <p>ジャンル： {{ $movie->genre ? $movie->genre->name : 'なし'}}</p>
            <a href="{{ route('admin.movies.edit', ['id' => $movie->id]) }}" class="btn btn-primary">映画詳細を編集する</a>
            <a href="{{ route('admin.movies.show', ['id' => $movie->id]) }}" class="btn btn-primary">映画のスケジュールを編集</a>
            </div>
        @endforeach
    </div>
